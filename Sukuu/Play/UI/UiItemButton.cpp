...
public:
	...
	void Tick(ActorBase& self, const ItemButtonParam& param)
	{
		...
		Circle(textPos, w / 4)
			...
			.draw(Color{U"#404040"});
		(void)FontAsset(AssetKeys::RocknRoll24)(U"{}"_fmt(param.index)).drawAt(textPos);
		...
		if (entered && not m_enteredBefore)
		{
			...
			if (isItemContaining) param.label.ShowMessage(FontAsset(AssetKeys::RocknRoll24)(itemProps.desc));
		}
